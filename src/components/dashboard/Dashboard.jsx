           ...
           <div className='  md:max-w-2/7 w-full'>
                {dashboardNav.map((item, index) => (
                  <Link key={item.id} to={item.href}>
                       <div className={`flex justify-between group/main cursor-pointer border-t border-t-gray-400 py-4 ${index === dashboardNav.length - 1 && 'border-b border-b-gray-400'}`}>
                      <span  className={` group-hover/main:text-black text-base ${active === item.title ? 'text-black font-semibold' : 'text-gray-500'}`}>{item.name}</span>
                      <span><item.icon size={20} className={` group-hover/main:text-black ${active === item.title ? 'text-black font-semibold' : 'text-gray-500'}`} /></span>
                    </div>
                  </Link>
                ))}
         </div>
