                        ...
                        <div className={`border-b border-b-gray-300 mb-5 relative  }`}>
                            ...
                            <div className='grid grid-cols-2 xl:grid-cols-5 gap-x-5 gap-y-4'>
                                {Categories?.products?.data?.slice(0,10).map((cat, index)=>(
                                    <Link to={`/category/${cat.slug ?? cat.name}`} key={cat.id} className='flex  items-center bg-white gap-3 cursor-pointer hover:shadow-lg rounded-xm px-1'>
                                       <div className='w-25 '>
                                         <img src={cat.image} className='' loading='lazy' alt={cat.name} />
                                       </div>
                                        <span className='font-inter text-[14px] text-[#333E48] hover:text-black '>{cat.name}</span>
                                    </Link>
                                ))}
                            </div>
                    </div>
