...
const ReusableNavbar = memo(( {data = [], setActiveMenu} ) => {
  ...
  return (
    <Container>
      <div className="bg-white dark:bg-[#262626] p-8 w-full border-t-2 dark:bg-[#181818] border-amber-400 shadow-xl flex flex-row gap-10 min-h-[420px]">
        ...
        {hasImg &&
         <div className='w-[260px] h-auto flex flex-col gap-4 shrink-0 items-center justify-center'>
            {Img.map((imgItem, index)=>(
                <Link key={imgItem.image.src} to={imgItem.image.url || '#'} className='relative overflow-hidden rounded-sm border border-gray-100 dark:border-[#333333] group/img'>
